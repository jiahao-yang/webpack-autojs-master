...
// Constants for ranking numbers to start and stop the process
const START_INDEX = 160;
const END_INDEX = 180;
    ...
    for (let i = startIndex; i <= endIndex; i++) {
        ...
        while (!userInteractionSuccessful && scrollCount <= SCROLL_ATTEMPTS) {
            user_ranking = className("android.widget.TextView")
                .text(`${i}`)
                .boundsInside(0, 0, device.width, device.height - EXCLUDED_HEIGHT_FROM_BOTTOM)
                .findOnce();
...
function main() {
    ...
    try {
        ...
        if (addAndLoop(START_INDEX, END_INDEX)) {
